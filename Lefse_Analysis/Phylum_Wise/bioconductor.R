...
org_data<-read.csv("complete_phylum.csv")
temp_cols_names<-names(org_data)

temp_cols_names<-temp_cols_names[5:length(temp_cols_names)-1]
print(temp_cols_names)

temp_rows_names<-org_data$X
print(temp_rows_names)

col_data<-org_data[1:nrow(org_data),c(5:ncol(org_data)-1)]
...
write.csv(res,"output_phylum.csv")
